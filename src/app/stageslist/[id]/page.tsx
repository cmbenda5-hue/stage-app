...
// ─── Stage card (lecture seule) ───────────────────────────────────────────────

function StageCard({ stage, onClick, onDownloadReport }: { stage: Stage; onClick: () => void; onDownloadReport: () => void }) {
  const [hovered, setHovered] = useState(false)
  return (
    <div
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{
        width: '100%', border: `1px solid ${hovered ? ORANGE : '#e5e7eb'}`,
        borderRadius: '18px', background: '#fff', padding: '16px 18px',
    ...
    >
      <button
        type="button"
        onClick={onClick}
        style={{ border: 'none', background: 'transparent', padding: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '14px', flex: 1, minWidth: 0, textAlign: 'left' }}
      >
        <div style={{
      ...
      {stage.report_url && (
        <button
          ...
          style={{
            flexShrink: 0, border: 'none', background: '#f0fdf4', color: '#16a34a', borderRadius: '999px',
            padding: '8px 12px', fontSize: '12px', fontWeight: 700, cursor: 'pointer',
            display: 'inline-flex', alignItems: 'center', gap: '5px',
          }}
        >
          <FileCheckIcon /> <DownloadIcon />
        </button>
      )}
    </div>
  )
...
export default function PublicStagesListPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        // Si l'utilisateur consulte sa propre liste via ce lien, on le renvoie sur sa page perso.
        if (user.id === studentId) { router.replace('/stageslist'); return }
        ...
        console.error('FETCH PUBLIC STAGES ERROR:', e)
        setError('Impossible de charger cette liste de stages pour le moment.')
      } finally {
        setLoading(false)
      }
    }
  ...
  async function handleDownloadReport(stage: Stage) {
    ...
    try {
      ...
      a.href = url
      a.download = `rapport-${stage.company_name.replace(/\s+/g, '-')}.pdf`
      document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url)
    } catch {
      alert("Téléchargement du rapport impossible.")
    }
  }

  const displayName = viewedProfile?.full_name || "l'étudiant·e"
    ...
    <main style={{ minHeight: '100vh', background: '#ffffff', fontFamily: 'Inter, system-ui, sans-serif', padding: '24px 16px', color: '#0f172a' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{
          ...
          <button
            type="button"
            onClick={() => router.push(studentId ? `/etudiantpublic/${studentId}` : '/dashboard')}
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '14px', padding: '4px 4px 0', flexWrap: 'wrap' }}>
          ...
          {/* Pas de bouton "Ajouter un stage" : liste en lecture seule */}
        </div>
        ...
        {loading ? (
          <div style={{ border: '1px solid #e5e7eb', borderRadius: '18px', background: '#fff', padding: '18px', color: '#64748b', fontSize: '14px', boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
            Chargement de la liste de stages...
          </div>
        ) : stages.length === 0 ? (
          <div style={{
            ...
            <div>
              <div style={{ fontSize: '15px', fontWeight: 800, color: '#0f172a' }}>Aucun stage pour le moment</div>
              <p style={{ margin: '6px 0 0', fontSize: '13px', color: '#64748b', maxWidth: '340px' }}>
                {displayName} n&apos;a pas encore ajouté de stage à sa liste.
              </p>
            </div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {stages.map((stage) => (
              <StageCard
                ...
                onDownloadReport={() => handleDownloadReport(stage)}
              />
            ))}
          </div>
        )}
